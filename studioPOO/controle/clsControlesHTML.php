...
class clsControlesHTML
{
	...
	public function geraGrid($tabela)
	{
		...
		$perfil = new clsFormaPagamento();

		while ($linha = mysqli_fetch_row($tabela)) {

			$select = $this->geraSelect('formapagamento_' . $linha[0], $perfil->pegaFormaPagamento(), $linha[4]);

			$htmlTabela .= '<tr>
								<td><img style="height:150px" src="imagens/' . $linha[2] . '.jpg"></img><br>
								<input type="file" size="30" name="txtArquivo_' . $linha[0] . '"/></td>
								<td><input type="text" size="20" name="txtNome_' . $linha[0] . '" value="' . $linha[1] . '"/>
								</td>
								<td><input type="text" size="10" readonly="true" class="login" name="txtLogin_' . $linha[0] . '" value="' . $linha[2] . '"/>
								</td>
